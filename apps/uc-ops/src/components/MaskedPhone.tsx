...
export function MaskedPhone({ phone, staticOnly }: Props) {
  ...
  return (
    <span className="uo-masked-phone">
      <span className="uo-masked-phone__value">{revealed ? phone : maskPhone(phone)}</span>
      {revealed ? (
        <span className="uo-masked-phone__countdown" aria-live="polite">
          {secondsLeft}s
        </span>
      ) : (
        <button
          type="button"
          className="uo-masked-phone__reveal"
          title="显示完整手机号"
          aria-label="显示完整手机号"
          onClick={() => {
            setRevealed(true);
